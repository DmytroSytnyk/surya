...
def to_sympy(latex):
    normalized = re.sub(r'operatorname\*{(\w+)}', '\1', latex)
...
class App(QMainWindow):
    ...
    def returnFromMimeData(self, urls):
        ...
        image_url = urls[0]
        if image_url and image_url.scheme() == 'file' \
                and image_url.fileName().split('.')[-1] in ACCEPTED_IMAGE_SUFFIX:
    ...
    def returnPrediction(self, result):
        self.toggleProcessing(False)
        success, prediction, confidence = result["success"], result["prediction"], result["confidence"]
        ...
        if success:
            ...
            self.error.setTextColor(Qt.GlobalColor.black)
            self.error.setText(f"Prediction confidence: {confidence}")
            self.retryButton.setEnabled(True)
        else:
            self.webView.setHtml("")
            msg = QMessageBox()
...
class ModelThread(QThread):
    ...
    def run(self):
        try:
            ...
            prediction = re.sub('<math( display=".*"|)>', '', prediction[0].text.replace('</math>', ''))
            prediction = prediction.replace('<', '\\lt ').replace('>', '\\gt ')
            self.finished.emit({"success": True, "prediction": prediction, "confidence": confidence})
        except Exception as e:
            import traceback
            traceback.print_exc()
            self.finished.emit({"success": False, "prediction": None, "confidence": None})
